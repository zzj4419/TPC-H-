...
public class formf3 extends JPanel implements ActionListener{
						...
						if("确认".equals(e.getActionCommand()))
						{
							...
							try {
					            ...
					            //要执行的SQL语句
					             ...
					             sql = "delete from partsupp where ps_partkey = "+shuxing+";";
					            
					            statement.execute(sql);
					            String[] shuxing2=jtf2.getText().split("//");
					            sql = "insert into partsupp values("+shuxing2[n].toString()+","+shuxing2[n+1].toString()+","+shuxing2[n+2].toString()+","+shuxing2[n+3].toString()+","+shuxing2[n+4].toString()+");";
					            statement.execute(sql);
