...
public class forma4 extends JPanel implements ActionListener{
...
//**************************************************************************
	...
	public forma4(){
		...
		String col[] = { "p_partkey", "p_name", "p_mfgr","p_type","p_brand","p_size","p_container","p_retailprice","p_comment"};
		String sn[][]=null; 
			...
			if("确认".equals(e.getActionCommand()))
			{
				...
				try {
		            ...
		            if(jtf1.getText().equals("*"))
		            	...
		            	{
		            	sql="select * from part where p_partkey ="+jtf1.getText()+";";
		                 rs =statement.executeQuery(sql);
		                 }
		           String col[] = { "p_partkey", "p_name", "p_mfgr","p_type","p_brand","p_size","p_container","p_retailprice","p_comment"};
		           if(rs.last())
					...
					if(row == 0){
						...
						{ 
							...
							sn[i][1] = rs.getString("p_name");       
							sn[i][2] = rs.getString("p_mfgr");       
							sn[i][3] = rs.getString("p_type");       
							sn[i][4] = rs.getString("p_brand");
							sn[i][5] = rs.getString("p_size");
							sn[i][6] = rs.getString("p_container");
							sn[i][7] = rs.getString("p_retailprice");
							sn[i][8] = rs.getString("p_comment");
							for(int k=0;k<9;k++)
							{
								if(sn[i][k]==null)
									sn[i][k]="无";
