...
public class formc4 extends JPanel implements ActionListener{
...
//**************************************************************************
	...
	public formc4(){
		...
		String col[] = { "S_SUPPKEY", "S_NAME", "S_ADDRESS","S_NATIONKEY","S_PHONE","S_ACCTBAL","S_COMMENT"};
		String sn[][]=null; 
			...
			if("确认".equals(e.getActionCommand()))
			{
				...
				try {
		            ...
		            if(jtf1.getText().equals("*"))
		            	...
		            	{
		            	sql="select * from supplier where S_SUPPKEY ="+jtf1.getText()+";";
		                 rs =statement.executeQuery(sql);
		                 }
		           String col[] = { "S_SUPPKEY", "S_NAME", "S_ADDRESS","S_NATIONKEY","S_PHONE","S_ACCTBAL","S_COMMENT"};
		           if(rs.last())
					...
					if(row == 0){
						...
						while(rs.next())       
						{ 
							sn[i][0] = rs.getString("S_SUPPKEY");       
							sn[i][1] = rs.getString("S_NAME");       
							sn[i][2] = rs.getString("S_ADDRESS");       
							sn[i][3] = rs.getString("S_NATIONKEY");       
							sn[i][4] = rs.getString("S_PHONE");
							sn[i][5] = rs.getString("S_ACCTBAL");
							sn[i][6] = rs.getString("S_COMMENT");
