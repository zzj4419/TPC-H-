...
public class formf4 extends JPanel implements ActionListener{
...
//**************************************************************************
	...
	public formf4(){
		...
		String col[] = { "PS_PARTKEY", "PS_SUPPKEY", "PS_AVAILQTY","PS_SUPPLYCOST","PS_COMMENT"};
		String sn[][]=null; 
			...
			if("确认".equals(e.getActionCommand()))
			{
				...
				try {
		            ...
		            if(jtf1.getText().equals("*"))
		            	...
		            	{
		            	sql="select * from partsupp where PS_PARTKEY ="+jtf1.getText()+";";
		                 rs =statement.executeQuery(sql);
		                 }
		           String col[] = { "PS_PARTKEY", "PS_SUPPKEY", "PS_AVAILQTY","PS_SUPPLYCOST","PS_COMMENT"};
		           if(rs.last())
					...
					if(row == 0){
						...
						while(rs.next())       
						{ 
							sn[i][0] = rs.getString("PS_PARTKEY");       
							sn[i][1] = rs.getString("PS_SUPPKEY");       
							sn[i][2] = rs.getString("PS_AVAILQTY");       
							sn[i][3] = rs.getString("PS_SUPPLYCOST");       
							sn[i][4] = rs.getString("PS_COMMENT");
