...
public class formh4 extends JPanel implements ActionListener{
...
//**************************************************************************
	...
	public formh4(){
		...
		String col[] = { "O_ORDERKEY", "O_CUSTKEY", "O_ORDERSTATUS","O_TOTALPRICE","O_ORDERDATE","O_ORDERPRIORITY","O_CLERK","O_SHIPPRIORITY","O_COMMENT"};
		String sn[][]=null; 
			...
			if("确认".equals(e.getActionCommand()))
			{
				...
				try {
		            ...
		            if(jtf1.getText().equals("*"))
		            	...
		            	{
		            	sql="select * from orders where O_ORDERKEY ="+jtf1.getText()+";";
		                 rs =statement.executeQuery(sql);
		                 }
		            String col[] = { "O_ORDERKEY", "O_CUSTKEY", "O_ORDERSTATUS","O_TOTALPRICE","O_ORDERDATE","O_ORDERPRIORITY","O_CLERK","O_SHIPPRIORITY","O_COMMENT"};
		           if(rs.last())
					...
					if(row == 0){
						...
						while(rs.next())       
						{ 
							sn[i][0] = rs.getString("O_ORDERKEY");       
							sn[i][1] = rs.getString("O_CUSTKEY");       
							sn[i][2] = rs.getString("O_ORDERSTATUS");       
							sn[i][3] = rs.getString("O_TOTALPRICE");       
							sn[i][4] = rs.getString("O_ORDERDATE");
							sn[i][5] = rs.getString("O_ORDERPRIORITY");
							sn[i][6] = rs.getString("O_CLERK");
							sn[i][7] = rs.getString("O_SHIPPRIORITY");
							sn[i][8] = rs.getString("O_COMMENT");
							for(int k=0;k<9;k++)
							{
								if(sn[i][k]==null)
									sn[i][k]="无";
